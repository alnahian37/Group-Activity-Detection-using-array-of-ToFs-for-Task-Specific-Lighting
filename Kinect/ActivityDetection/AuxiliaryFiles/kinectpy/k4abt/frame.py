...
from kinectpy.k4abt import body
from kinectpy.k4abt.body import Body
from kinectpy.k4abt.body2d import Body2d
from kinectpy.k4abt.body3d import Body3d
from kinectpy.k4abt._k4abtTypes import k4abt_body_t, body_colors
from kinectpy.k4a import Image, Capture, Transformation
...
class Frame:
	...
	def get_body3d(self, bodyIdx = 0, dest_camera = K4A_CALIBRATION_TYPE_DEPTH):
		
		body_handle = self.get_body(bodyIdx).handle()
		return Body3d.create(body_handle, self.calibration, bodyIdx, dest_camera)

	def render_bodies3d(self, canvas, dest_camera = K4A_CALIBRATION_TYPE_DEPTH):
		num_bodies = self.get_num_bodies()
		
		for body_id in range(num_bodies):
			canvas = self.render_body3d(canvas, body_id, dest_camera)
		
		return canvas

	def render_body3d(self, destination_image, bodyIdx = 0, dest_camera = K4A_CALIBRATION_TYPE_DEPTH):
		return self.get_body3d(bodyIdx, dest_camera).render(destination_image)
