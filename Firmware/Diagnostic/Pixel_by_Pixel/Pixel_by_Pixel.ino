...
// Define the LED to change value
int thisPin = 0;
...
void send_init_reg(unsigned int *reg)
{
  ...
  for(int i=0; i<34; i++) SPI.transfer16(reg[i]);
  SPI.transfer16(0);
  delayMicroseconds(SPI_CS_DELAY);
  chipUpdate(1);
  delayMicroseconds(SPI_CS_DELAY);
...
// This sends 3 words to the LED driver and a zero
...
void initDev()
{
  ...
  unsigned int intReg_HT_iW7039[34] = {   0x8020,0x60, // broadcast command with same data writting for 32 words for HT
                                          0x800, 0x800, 0x800, 0x800, 0x800, 0x800, 0x800, 0x800, // 0x60 ~ 0x67
                                          0x800, 0x800, 0x800, 0x800, 0x800, 0x800, 0x800, 0x800, // 0x68 ~ 0x6F
                                          0x800, 0x800, 0x800, 0x800, 0x800, 0x800, 0x800, 0x800, // 0x70 ~ 0x77
                                          0x800, 0x800, 0x800, 0x800, 0x800, 0x800, 0x800, 0x800 // 0x78 ~ 0x7F
                                      }; //0x60 to 0x7F
...
void loop()
{
  ...
  for(byte i=0; i<49; i++)
  {
    Serial.println((String)"Pixel - "+i);
    send_3words_plus_zero(red_cmd_1[i], red_cmd_2[i], red_cmd_3[i]);
    send_3words_plus_zero(green_cmd_1[i], green_cmd_2[i], green_cmd_3[i]);
    send_3words_plus_zero(blue_cmd_1[i], blue_cmd_2[i], blue_cmd_3[i]);
    delay(1000);
